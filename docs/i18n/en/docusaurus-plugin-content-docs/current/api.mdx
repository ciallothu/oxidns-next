...
The OxiDNS Next management API is a separate control plane for health checks, configuration validation and persistence, process control, logs, upgrades, plugin extension APIs, and Prometheus metrics. This page is the interface map; detailed request formats, response fields, and examples live in topic pages.
...
These endpoints use the same request and response shapes for SQLite, PostgreSQL, and MySQL. When `query_recorder.args.api_cache` is configured, successful record-list, detail, and statistics responses may be served briefly from Redis; an unavailable or invalid cache falls through to the SQL database.

#### `GET /api/plugins/<tag>/records`

Returns lightweight record summaries ordered by `created_at_ms` descending. It omits `steps` and the full DNS RR arrays. `answer_preview` contains at most four short answers so the list can show resolved IP, CNAME, and similar values directly; use the detail endpoint for the complete response.
...
Returns one full record plus its `steps` array. The full record includes request/response flags, EDNS data, and the `answers_json`, `authorities_json`, `additionals_json`, and `signature_json` RR arrays.

Responses:

* `200 OK`
  * JSON containing a `record` object. Full record fields live directly on that object and `record.steps` holds path events.
* `404 Not Found`
  * The record does not exist.

#### `DELETE /api/plugins/<tag>/records`

Clears all persisted query-history rows and `steps` path events for the current recorder. The operation first flushes the background writer queue, deletes this recorder's rows from the selected SQL backend, and clears the in-memory tail and matching Redis API-cache namespace.
...
* `limit=<n>`
  * Number of buckets to return. Defaults to `20`. The backend no longer enforces a `200` cap; large values increase database sorting and response-size cost.
...
Buckets are aligned to UTC. Month buckets follow calendar months, including leap years; response `bucket_ms=0` denotes variable month lengths. Each point's `bucket_ms` is always its start Unix timestamp in milliseconds. Response `since_ms` / `until_ms` report the effective query bounds. The default end is now; `buckets` bounds the most recent bucket count and empty buckets are zero-filled. Trends aggregate all retained records in the selected range, without the 10,000-record sampling cap. Mean latency is weighted and P95 uses the nearest rank.

The WebUI defaults to 24 hours and supports 1 hour / 24 hours / 7 days / 30 days / 12 calendar months, using minute / hour / day / day / month buckets. Use the slider or Ctrl + wheel to zoom and drag to pan. Empty latency buckets remain gaps; counts are zero. History depends on retention: set `retention_days` to at least 366 in advance for a full year. Already deleted records cannot be reconstructed.
