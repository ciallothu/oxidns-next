...
OxiDNS Next usually accepts both decimal numbers and mnemonic names in `qtype`, `qclass`, and `rcode` matchers, as well as in `reject [rcode]`. Mnemonic names are case-insensitive, so `SERVFAIL`, `servfail`, and `ServFail` all refer to the same RCODE.
...
If a type is not listed here, match it with its decimal number. If OxiDNS Next has a built-in name for that type, the mnemonic name is accepted too.
