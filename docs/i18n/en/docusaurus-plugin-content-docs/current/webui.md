...
- Query Log lives at `/query-log`. It uses `query_recorder` history from the configured SQLite, PostgreSQL, or MySQL database plus live events, with domain or client-address keyword and date-range filters. The list shows resolved answers directly, while details show the complete response and a naturally scrolling static execution flow. The default `config.yaml` uses SQLite with seven-day retention. An optional Redis short cache can accelerate record-list, detail, and statistics APIs and automatically falls back to SQL when unavailable. The database contains client addresses, domains, and response data and must be treated as sensitive. Local SQLite files are tightened to mode `0600` when opened or created on Unix; external databases require operator-managed TLS, account privileges, network access controls, and backups.
