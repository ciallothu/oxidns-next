...
- Decimal numeric rcodes and case-insensitive English RCODE names are accepted, for example `reject 2`, `reject SERVFAIL`, `reject servfail`, or `reject NXDOMAIN`.
- `reject 0` returns a plain `NOERROR` response and does not add an SOA automatically.
- `reject 0 soa` or `reject noerror soa` returns a NODATA-style `NOERROR` response: the Answer section is empty, and the Authority section includes a synthetic SOA.
