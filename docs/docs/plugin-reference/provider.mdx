...
- 类型：`array`；必填：否；默认值：空数组
- 作用：指定外部规则文件路径列表。
- 示例：`- "/etc/oxidns-next/domains.txt"`
- 文件要求：
  - 每行一条规则。
  - 空行与注释行会被忽略。
- 运行影响：
...
`dynamic_domain_set` 最初在上游 OxiDNS `v1.2.0` 发布，OxiDNS Next 继承后仍将其视为**实验功能**：相关配置可能在后续版本调整。生产部署前请评估学习速率与磁盘写入对持久化层的影响。
...
- 如需在运行前把部分 selector 预导出成文本规则文件，可使用 `oxidns-next export-dat --kind geosite`。
...
- 可通过 `reload_provider` 或 `POST /plugins/<tag>/reload` 独立刷新 dat 文件内容。
- 如需在运行前把部分 selector 预导出成文本规则文件，可使用 `oxidns-next export-dat --kind geoip`。
