...
- 默认 `rcode` 为 `REFUSED`。
- 支持十进制数值和大小写不敏感的英文 RCODE 名称，例如 `reject 2`、`reject SERVFAIL`、`reject servfail`、`reject NXDOMAIN`。
- `reject 0` 返回普通 `NOERROR` 响应，不会自动附加 SOA。
- `reject 0 soa` 或 `reject noerror soa` 返回 NODATA 风格的 `NOERROR` 响应：Answer 为空，Authority 区会加入 synthetic SOA。
