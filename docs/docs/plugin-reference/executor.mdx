...
执行器是 OxiDNS Next 的核心动作层。它们可以读写请求、设置响应、调用上游、缓存结果、做回退、记录日志，或触发系统联动。
...
- 模式说明：
  ...
  - `balanced`：默认模式。`NOERROR` 且包含答案时立即胜出；`NXDOMAIN` 或 NODATA 会短暂等待是否有正向答案。
...
- 规则说明：
  ...
  - 使用 `bootstrap` 后，上游域名解析由 OxiDNS Next 查询该引导服务器完成，并按 TTL 缓存。
...
- 建议：开启前先用 `oxidns-next probe upstream <addr>` 探测目标上游。该命令会对 TCP/DoT 强制同一条连接并发查询，帮助判断该上游是否适合启用 pipeline，避免遇到超时、连接关闭、响应 ID 错乱或 question 串线。
...
- tag: cache_main
  ...
  args:
    ...
    # 定期落盘周期，单位秒
    dump_interval: 600
    # 可选 Redis 二级缓存；需先配置顶层 storage.redis
    redis:
      enabled: true
...
- 类型：`object`；必填：否；默认：关闭
- 作用：在进程内 L1 之外启用共享 Redis L2。需先配置顶层 [`storage.redis`](../configuration.md#storage)。
- `enabled`：是否启用；配置该对象时默认 `true`。
- `command_timeout_ms`：单次 Redis 命令超时，默认 `20`。
- `max_inflight`：允许同时执行的 Redis 命令数，默认 `64`。
- `write_queue_size`：异步写入队列大小，默认 `4096`。
- `failure_threshold`：连续失败多少次后打开熔断器，默认 `3`。
- `retry_after_ms`：熔断后再次试探 Redis 的等待时间，默认 `30000`。
- Redis 不可用、超时、队列满或熔断时，查询不会失败，而是继续使用进程内缓存和上游。Redis key 不明文保存 qname 或客户端 IP。
...
- 内置过期清理和近似 LRU 回收。
- 启用 Redis 时，进程内缓存仍是 L1；Redis 只是可丢弃的共享 L2，不改变 DNS TTL、负缓存或 lazy cache 规则。
...
- `cache_expired_total`
- `cache_insert_total`
- `cache_skip_total{reason="truncated|no_ttl|low_positive_ttl"}`
...
- `dump_file` 是本地缓存快照；Redis L2 与 dump 互不替代。不要把 Redis 当作唯一持久化副本。
...
<Admonition type="warning" title="实验性插件（继承自上游 OxiDNS v1.2.0）">

`ip_selector` 最初在上游 OxiDNS `v1.2.0` 发布，OxiDNS Next 继承后仍将其视为**实验功能**：配置字段、默认值、探测策略与缓存语义可能在后续版本根据实际使用反馈调整。建议在非关键链路先行验证，关注 release notes 中的相关变更说明。
...
`ip_selector` 只接受上面列出的 OxiDNS Next 原生命名，不提供兼容别名。
...
`learn_domain` 最初在上游 OxiDNS `v1.2.0` 发布，OxiDNS Next 继承后仍将其视为**实验功能**：过滤条件、写入触发时机与配套 `dynamic_domain_set` 的接口可能在后续版本调整。生产部署前请评估学习速率与磁盘写入对持久化层的影响。
...
- tag: learned_allow
  ...
  args:
    path: "/etc/oxidns-next/learned-allow.txt"
...
把入口 request、执行后 response 以及 `sequence` 路径事件持久化到 recorder 配置的 SQLite、PostgreSQL 或 MySQL 数据库，并暴露历史查询、统计和 SSE 实时推送接口。可选 Redis 只缓存短时 API 响应，不替代 SQL 持久化。

仓库默认 `config.yaml` 已启用 `query_log`，并把它放在 `main_sequence` 的第一步，使其 `with_next` 包裹完整的下游请求/响应链。自定义链路时也应把 recorder 放在希望观察的执行范围之前。
...
- tag: query_recorder_main
  ...
  args:
    ...
    # 可选 Redis API 短缓存；需先配置顶层 storage.redis
    api_cache:
      enabled: true
      records_ttl_ms: 2000
      stats_ttl_ms: 5000
...
- 类型：`object`；必填：新配置是
- 作用：选择查询记录的 SQL 持久化后端。
- `type: sqlite`
  - `path`：必填，本地数据库文件路径。
  - `acquire_timeout_ms`：可选，默认 `3000`，范围 `1..=300000`。
  - `query_timeout_ms`：可选，默认 `20000`，范围 `1..=300000`。
- `type: postgres` 或 `type: mysql`
  - `url`：必填。PostgreSQL 接受 `postgres://` 或 `postgresql://`，MySQL 使用 `mysql://`。推荐写成 `${OXIDNS_NEXT_QUERY_DATABASE_URL}`，通过环境变量提供凭据。
  - `max_connections`：可选，默认 `8`，范围 `2..=256`。
  - `connect_timeout_ms`：可选，默认 `5000`，范围 `1..=300000`。
  - `acquire_timeout_ms`：可选，默认 `3000`，范围 `1..=300000`。
  - `query_timeout_ms`：可选，默认 `20000`，范围 `1..=300000`。
...
MySQL 使用相同连接池字段，只需改为 `type: mysql` 并提供 `mysql://` URL。目标数据库必须预先存在；连接账号需要在该数据库中创建/迁移表以及读写、清理记录的权限。插件只创建自己的版本化表，不会创建数据库。完整 Compose 示例见 [`examples/storage`](https://github.com/ciallothu/oxidns-next/tree/main/examples/storage)。

#### `path`（旧写法）

- 类型：`string`；必填：否
- 作用：兼容旧 SQLite 配置；等价于 `database: { type: sqlite, path: ... }`。
- 限制：不能和 `database` 同时配置。新配置应使用 `database`。
...
- 最小值：`1`
- 作用：限制 WebUI / API 统计与历史查询的同时读取数。PostgreSQL/MySQL 下必须小于 `database.max_connections`，为写入与清理保留连接池容量。

#### `api_cache`

- 类型：`object`；必填：否；默认：关闭
- 作用：使用顶层 `storage.redis` 缓存成功的记录列表、详情与统计 API JSON 响应。Redis 不保存查询记录，也不参与写入队列。
- `enabled`：是否启用。
- `records_ttl_ms`：记录列表缓存时间，默认 `2000`。
- `stats_ttl_ms`：统计结果缓存时间，默认 `5000`。
- `command_timeout_ms`：单次 Redis 命令超时，默认 `100`。
- `max_value_bytes`：单个缓存值上限，默认 `1048576`。
- Redis 连接失败、超时、熔断或缓存值无效时，请求会直接回退到 SQL 数据库。
...
- request / response 不保存 wire 数据，而是把问题区、RR、EDNS 等字段拆成结构化 JSON 列。
- `client_ip` 字段来自 OxiDNS Next 看到的传输层来源地址；如果前面有 systemd-resolved、dnsmasq、AdGuardHome、dae、clash 等本机转发链路，记录中可能只看到 `127.0.0.1`。
...
- 每个 recorder 独占自己的有界队列、数据库后端、内存 tail 和 SSE 广播器。
- 表名前缀包含 plugin tag 的安全片段和完整哈希；同一数据库中的多个 recorder 使用不同表，不会互相覆盖。SQLite 仍可为不同 recorder 配置不同文件。
...
- `answer_preview` 是列表接口使用的有界摘要：最多保留前 4 条 Answer，每条 `payload_text` 最多 160 个 Unicode 字符，并通过 `truncated` 标记截断；详情接口仍返回上述完整 RR 数组。已有 SQLite 行不会在启动时做大表回填，因此旧行的预览可为空，但完整详情不受影响。
- `req_edns_json`、`resp_edns_json` 固定为 EDNS 对象或 `NULL`。
- 表名中的 `v1` 是当前逻辑 schema 版本；SQLite 的兼容性附加列会在启动时自动补齐，但不会在 SQLite、PostgreSQL 与 MySQL 之间迁移日志。

### API

- `GET /plugins/<tag>/records`
  - 按 `created_at_ms` 倒序分页返回轻量摘要与 `answer_preview`，不读取完整 RR 数组或 `steps`。
  - 查询参数：
...
- 若未启用管理 API，recorder 仍会写入所选 SQL 数据库，但不会暴露查询与 SSE 路由。
- SQLite 文件在 Unix 上创建或打开时会收紧为 `0600`；PostgreSQL/MySQL 的网络访问控制、TLS、账号权限与备份由数据库部署负责。
- 若需要真实终端 IP，请让客户端直连 OxiDNS Next，或在 HTTP/DoH 反向代理场景配置可信的 `src_ip_header`。
...
- tag: ipset_main
  ...
  args:
    # A 记录写入的 ipset
    set_name4: "oxidns-next_v4"
    # AAAA 记录写入的 ipset
    set_name6: "oxidns-next_v6"
    # IPv4 写入前先聚合成 /24
    mask4: 24
    # IPv6 写入前先聚合成 /64
    mask6: 64
...
```yaml
- exec: "ipset oxidns-next_v4,inet,24 oxidns-next_v6,inet6,64"
```
...
- tag: ros_address_list_main
  ...
  args:
    ...
    address_list4: "oxidns-next_ipv4"
    # AAAA 记录写入的 address-list
    address_list6: "oxidns-next_ipv6"
    # 用于标记 OxiDNS Next 管理条目的注释前缀
...
- 建议使用 OxiDNS Next 专用 `address-list`，避免接入大型共享列表，以降低 RouterOS 管理面扫描成本。
...
执行 OxiDNS Next 升级流程，可用于 `cron`、`sequence` 或其它执行器触发的维护任务。
...
- `repository`
    - GitHub 仓库，默认 `ciallothu/oxidns-next`。
- `asset`
...
```yaml
- exec: "download https://example.com/rules.txt /etc/oxidns-next"
```
...
- tag: provider_geosite
  type: geosite
  args:
    file: "/etc/oxidns-next/geosite.dat"
...
plugins:
  ...
  - tag: provider_geosite
    type: geosite
    args:
      file: "/etc/oxidns-next/rules/geosite.dat"

  - tag: provider_geoip
    type: geoip
    args:
      file: "/etc/oxidns-next/rules/geoip.dat"
```
...
- 需要运行在带有应用控制上下文的正常 OxiDNS Next 进程中。
