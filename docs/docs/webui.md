...
- 查询日志位于 `/query-log`，由 `query_recorder` 提供 SQLite 历史和实时流，可按域名或客户端地址关键词以及日期范围筛选。默认 `config.yaml` 已启用一个 7 天保留期的 recorder。查询库包含客户端地址、域名与响应内容，应按敏感数据保护；Unix 上创建或打开时会将数据库权限收紧为 `0600`。
