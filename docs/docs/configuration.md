...
- `reject 0 soa` 或 `reject noerror soa` 返回 NODATA 风格的 `NOERROR` 响应：Answer 为空，Authority 区会加入一个 synthetic SOA，用于让客户端和递归缓存明确这是“该类型无答案”，而不是域名不存在。
- `soa` 参数只支持和 `rcode 0` / `NOERROR` 一起使用，不支持 `reject 3 soa` 或 `reject NXDOMAIN soa` 这类写法。
- `reject 0 soa` / `reject NOERROR soa` 中 SOA 记录的 class 会跟随当前 question 的 class。
