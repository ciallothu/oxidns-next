...
本页先列出 OxiDNS Next 的当前计划和版本里程碑，再保留上游 OxiDNS 的历史路线，避免将上游版本号误认为本项目版本。

<RoadmapTimeline>

<RoadmapItem type="future" label="敬请期待" title="简单模式 WebUI" desc="基于模板配置，打造类 AdGuard Home 的开箱即用体验" num={3}>

面向不想编写 YAML 的普通用户，提供一套预置场景模板（去广告、防污染、家庭过滤、分流加速等），通过表单与开关完成主要配置；保留切回完整模式继续手写规则的入口。目标体验对标 AdGuard Home 的简单管理界面，让 OxiDNS Next 的安装门槛接近"开箱即用"。
...
<RoadmapItem type="version" title="OxiDNS Next 首发" desc="独立品牌、多方式认证、独立查询日志与统一插件仪表盘" version="v0.1.0" date="2026-07-17">

建立 OxiDNS Next 的独立仓库与发布渠道；新增本地登录、OIDC、通行密钥和 TOTP；将可检索的查询日志从系统日志中分离；并把仪表盘和插件中心整合为一个工作区。
...
以下记录来自 [SvenShi/oxidns](https://github.com/svenshi/oxidns)，用于说明本 fork 继承代码的演进背景，不代表 OxiDNS Next 的版本序列。
...
<div style={{borderLeft: '4px solid var(--ifm-color-primary)', background: 'rgba(15, 118, 110, 0.06)', borderRadius: '0 12px 12px 0', padding: '0.9rem 1.2rem', marginTop: '2rem'}}>
  ...
  <ul style={{margin: '0.5rem 0 0', paddingLeft: '1.25rem', lineHeight: 1.75}}>
    <li><strong>WebAssembly 插件</strong>：探索通过 WASM 支持第三方插件，允许开发者用任意语言按规范开发和分发插件，无需修改 OxiDNS Next 主体代码，并天然获得沙箱隔离保障。</li>
    <li><strong>动态链接库插件</strong>：探索通过动态库（.so / .dylib）加载机制支持原生插件，适合对性能要求极高的场景，开发者可独立编译和分发，OxiDNS Next 在运行时按需加载。</li>
  </ul>
</div>
