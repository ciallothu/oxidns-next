...
import {
  ...
  getOxiDnsNextConfigSubKeys,
  getOxiDnsNextConfigValueSuggestions,
  OXIDNS_LOG_LEVELS,
  type OxiDnsNextConfigValueSuggestion,
...
export type OxiDnsNextYamlTheme = "dark" | "light";

interface OxiDnsNextYamlPalette {
  activeLine: string;
...
const lightPalette: OxiDnsNextYamlPalette = {
  activeLine: "#eef6ff",
...
const darkPalette: OxiDnsNextYamlPalette = {
  activeLine: "#263449",
...
function paletteFor(theme: OxiDnsNextYamlTheme) {
...
export function oxidnsYamlSyntaxHighlighting(theme: OxiDnsNextYamlTheme) {
  return [
    syntaxHighlighting(
...
export function applyOxiDnsNextYamlDiagnostics(
  view: EditorView,
  context: OxiDnsNextYamlEditorContext,
  backendDiagnostics: OxiDnsNextYamlDiagnostic[] = [],
) {
...
function keySuggestions(
  ...
  if (context.variant === "config") {
    const subKeys = getOxiDnsNextConfigSubKeys(path);
    if (subKeys !== null) {
...
function pluginKindSuggestions(context: OxiDnsNextYamlEditorContext): Completion[] {
  const locale = contextLocale(context);
...
function logLevelSuggestions(context: OxiDnsNextYamlEditorContext): Completion[] {
  const locale = contextLocale(context);
...
function configValueSuggestions(
  ...
  const suggestions =
    valueKey === "level" && path.includes("log")
      ...
      : getOxiDnsNextConfigValueSuggestions(path, valueKey).map(
...
function configValueCompletion(
  suggestion: OxiDnsNextConfigValueSuggestion,
): Completion {
  return {
