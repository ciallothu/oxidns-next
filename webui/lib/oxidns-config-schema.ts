...
export interface OxiDnsNextConfigValueSuggestion {
  label: string;
  apply?: string;
  detail?: string;
...
export function getOxiDnsNextConfigSubKeys(path: string[]): string[] | null {
...
export function getOxiDnsNextConfigValueSuggestions(
  path: string[],
  valueKey: string | null,
): OxiDnsNextConfigValueSuggestion[] {
  if (!valueKey) return [];
  const normalized = normalizeConfigPath(path);
...
export function sortOxiDnsNextConfigForSerialize(value: unknown): unknown {
...
function booleanSuggestions(): OxiDnsNextConfigValueSuggestion[] {
  return [
