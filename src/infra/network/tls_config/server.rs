...
pub fn load_tls_config(
    cert: &Option<String>,
    key: &Option<String>,
) -> Option<crate::infra::error::Result<ServerConfig>> {
    match (cert, key) {
        (Some(cert), Some(key)) => {
...
) -> crate::infra::error::Result<Option<ServerConfig>> {
    match (cert, key) {
        (Some(cert), Some(key)) => {
...
fn load_tls_config_from_path(
    cert_path: &str,
    key_path: &str,
) -> crate::infra::error::Result<ServerConfig> {
...
fn load_certificates(cert_path: &str) -> crate::infra::error::Result<Vec<CertificateDer<'static>>> {
